...
// ID returns the ID of the RawNode.
func (rn *RawNode) ID() uint64 {
	return rn.raft.id
}

// HasProgress reports whether the RawNode tracks progress for the given ID.
func (rn *RawNode) HasProgress(id uint64) bool {
	return rn.raft.trk.Progress[id] != nil
}

// AsyncStorageWritesEnabled reports whether asynchronous storage writes are
// enabled for the RawNode.
func (rn *RawNode) AsyncStorageWritesEnabled() bool {
	return rn.asyncStorageWrites
}

// Logger returns the logger configured for the RawNode.
func (rn *RawNode) Logger() Logger {
	return rn.raft.logger
}
...
func (rn *RawNode) Step(m *pb.Message) error {
	...
	return rn.stepForNode(m)
}

// stepForNode advances the state machine for the Node actor. Unlike Step, it
// accepts the local messages generated by Node methods. It retains RawNode's
// filtering of response messages from unknown peers.
...
func (rn *RawNode) ForgetLeader() error {
	return rn.raft.Step(&pb.Message{Type: pb.MsgForgetLeader.Enum()})
}
